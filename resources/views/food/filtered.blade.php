    ...
    @foreach($foods as $food)
        <div class="max-w-sm bg-gray-100 rounded overflow-hidden shadow-lg" id="food-card-{{ $food->id }}">
            ...
            <div class="px-6 pt-2 pb-2 grid grid-cols-2 gap-2">
                <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">#{{ FoodCategory::query()->find($food->food_category_id)->name }}</span>
                <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">Price: {{ $food->price }}</span>
                <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">Discount: {{ $food->discount }}</span>
                <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">Status: {{ $food->status == 1 ? "Available" : "Not Available" }}</span>
            </div>
            <div class="flex mt-4">
                ...
                @if (!FoodParty::query()->where('food_id', $food->id)->first())
                    <form action="{{ route('food-party.store', [$restaurant, $food]) }}" method="post">
                        ...
                        <div>
                            ...
                            <x-primary-button class="bg-pink-500 hover:bg-pink-700 text-white font-bold py-2 px-4 m-3 rounded">
